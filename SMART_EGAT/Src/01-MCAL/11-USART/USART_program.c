...
u8 USART_u8ReceiveByteNonBlocking(char* data,USART_Num_t Copy_USARTNumber) {
    // Check if the data is available in the USART data register
    if (USART[Copy_USARTNumber]->SR & (1 << 5)) { // Check if RXNE (Receive Data Register Not Empty) flag is set
        // Read the data from the USART data register
        *data = USART[Copy_USARTNumber]->DR;  // Store the received byte in the provided buffer
        return 1;  // Return 1 to indicate that data is received
    }
